...
pub struct Merchant {
    ...
    pub current_day_volume: i128,
    pub current_month_volume: i128,
...
impl MerchantNetwork {
    pub fn register_merchant(
        ...
        env.storage().instance().set(&MerchantKey::Merchants, &merchants);

        let mut queue: Vec<String> = env
            .storage()
            .instance()
            .get(&MerchantKey::MerchantQueue)
            .unwrap_or(Vec::new(&env));
        queue.push_back(merchant_id);
        env.storage().instance().set(&MerchantKey::MerchantQueue, &queue);
    }
    ...
    pub fn process_payment(
        ...
        if amount <= 0 || amount > merchant.daily_limit || amount > merchant.monthly_limit {
            panic!("limit exceeded");
        }

        merchant.current_day_volume = amount;
        merchant.current_month_volume = amount;
        merchants.set(merchant_id.clone(), merchant);
        env.storage().instance().set(&MerchantKey::Merchants, &merchants);

        let mut all_txs: Map<String, Vec<Transaction>> = env
            .storage()
            .instance()
            .get(&MerchantKey::MerchantTxs)
            .unwrap_or(Map::new(&env));

        let mut txs = all_txs.get(merchant_id.clone()).unwrap_or(Vec::new(&env));
        ...
        all_txs.set(merchant_id, txs);
        env.storage().instance().set(&MerchantKey::MerchantTxs, &all_txs);
    ...
    pub fn get_merchant_transactions(env: Env, merchant_id: String) -> Vec<Transaction> {
        let all_txs: Map<String, Vec<Transaction>> = env
            .storage()
            .instance()
            .get(&MerchantKey::MerchantTxs)
            .unwrap_or(Map::new(&env));
        all_txs.get(merchant_id).unwrap_or(Vec::new(&env))
    }
    ...
    pub fn update_reputation(
        ...
        if let Some(mut merchant) = merchants.get(merchant_id.clone()) {
            if feedback_score >= 0 {
                merchant.reputation_score = (merchant.reputation_score + feedback_score as u32).min(100);
            } else {
                merchant.reputation_score = merchant.reputation_score.saturating_sub((-feedback_score) as u32);
            }
            merchants.set(merchant_id, merchant);
            env.storage().instance().set(&MerchantKey::Merchants, &merchants);
        }
